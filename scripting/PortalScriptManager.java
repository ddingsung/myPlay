...
public class PortalScriptManager {
    ...
    private final static ScriptEngineFactory sef = new ScriptEngineManager().getEngineByName("rhino").getFactory();
    ...
    public final void executePortalScript(final MaplePortal portal, final MapleClient c) {
        ...
        if (script != null) {
            try {
                ...
                if (c.getPlayer().isGM()) {
                    c.getPlayer().dropMessage(6, "[포탈] : " + portal.getScriptName());
                }
            } catch (Exception e) {
