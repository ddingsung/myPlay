...
public class EventInstanceManager {
    ...
    private ScheduledFuture<?> ElapsedTimer = null;
    private AtomicInteger elapsedtime = new AtomicInteger(0);
    private List<ScheduledFuture<?>> Schedules = new ArrayList<ScheduledFuture<?>>();
    ...
    public void allCancelSchedules() {
        for (ScheduledFuture<?> sche : Schedules) {
            if (sche != null) {
                if (sche.isCancelled() || sche.isDone()) {
                    continue;
                } else {
                    sche.cancel(false);
                }
    ...
    public void startElapsedTimer() {
        ...
        ElapsedTimer = EventTimer.getInstance().register(new Runnable() {
            public void run() {
                if (disposed || em == null) {
                    return;
                }
                try {
                    em.getIv().invokeFunction("elapsedTime", EventInstanceManager.this, elapsedtime.incrementAndGet());
                } catch (NoSuchMethodException ignore) {
                    //ignore
                } catch (Exception ex) {
                    FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Event name" + em.getName() + ", Instance name : " + name + ", method Name : elapsedTime:\n" + ex);
                    System.out.println("Event name" + em.getName() + ", Instance name : " + name + ", method Name : elapsedTime:\n" + ex);
                }
            }
        }, 1000);
    }
        ...
        try {
            if (chars != null && chars.size() <= size) {
                ...
                for (MapleCharacter chr : chrs) {
                    ...
                    unregisterPlayer_NoLock(chr);
                    if (towarp > 0 && chr.getMapId() != towarp) {
                        if (chr.isAlive()) {
                            chr.changeMap(map, map.getPortal(0));
                        } else {
                            chr.changeMap(map.getForcedReturnMap(), map.getPortal(0));
                        }
                    }
                }
                dispose_NoLock();
    ...
    public boolean revivePlayer(MapleCharacter chr) {
        ...
        try {
            ...
            //Ignore... -> too lazy make this into all events
        } catch (Exception ex) {
    ...
    public void playerDisconnected(final MapleCharacter chr, int idz) {
        ...
        byte ret;
        try {
            Object b = em.getIv().invokeFunction("playerDisconnected", this, chr);
            if (b instanceof Boolean) {
                return;
            } else {
                ret = (byte) Integer.parseInt(b.toString());
            }
        } catch (Exception e) {
            ret = 0;
        }
    ...
    public void monsterDamaged(final MapleCharacter chr, final MapleMonster mob, final int damage) {
        ...
        try {
            em.getIv().invokeFunction("monsterDamaged", this, chr, mob, damage);
        } catch (ScriptException ex) {
            System.out.println("Event name" + (em == null ? "null" : em.getName()) + ", Instance name : " + name + ", method Name : monsterDamaged:\n" + ex);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Event name" + (em == null ? "null" : em.getName()) + ", Instance name : " + name + ", method Name : monsterDamaged:\n" + ex);
        } catch (NoSuchMethodException ex) {
            System.out.println("Event name" + (em == null ? "null" : em.getName()) + ", Instance name : " + name + ", method Name : monsterDamaged:\n" + ex);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Event name" + (em == null ? "null" : em.getName()) + ", Instance name : " + name + ", method Name : monsterDamaged:\n" + ex);
        } catch (Exception ex) {
    ...
    public void broadcastEffect(String data) {
        if (disposed) {
            return;
        }
        for (MapleCharacter chr : getPlayers()) {
            chr.getClient().getSession().write(MaplePacketCreator.showEffect(data));
        }
    }
    ...
    public final void broadcastGhostTeamPacket(int type, String object, String amount) {
        if (disposed) {
            return;
        }
        for (MapleCharacter chr : getPlayers()) {
            chr.getClient().getSession().write(MaplePacketCreator.sendString(type, object, amount));
        }
    }

    public final void broadcastGhostTeamPacket(int type, String object, String amount, int team) {
        if (disposed) {
            return;
        }
        for (MapleCharacter chr : getPlayers()) {
            MapleCarnivalParty cpt = chr.getCarnivalParty();
            if (cpt != null && cpt.getTeam() == team) {
                chr.getClient().getSession().write(MaplePacketCreator.sendString(type, object, amount));
            }
        }
    }
    ...
    public final void schedule(final String methodName, final long delay) {
        ...
        Schedules.add(EventTimer.getInstance().schedule(new Runnable() {
            public void run() {
    ...
    public final void registerRaidParty(final MapleCharacter chr, final int team) {
        if (disposed) {
            return;
        }
        ...
        new MapleCarnivalParty(chr, characters, (byte) team);
        try {
            em.getIv().invokeFunction("registerRaidParty", this, characters, team);
        } catch (ScriptException ex) {
            System.out.println("Event name" + em.getName() + ", Instance name : " + name + ", method Name : registerRaidParty:\r\n" + ex);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "Event name" + em.getName() + ", Instance name : " + name + ", method Name : registerRaidParty:\r\n" + ex);
        } catch (NoSuchMethodException ex) {
            //ignore
        }
    }
    ...
    public void gainExpR(final MapleCharacter chr, final int exp) {
        chr.gainExp(exp * RateManager.PEXP, true, true, true);
    }

    public final void gainPartyExpPQ(MapleMap map, int exp, String pq, int mod) {
        int qid = -1;
        ...
        for (final MapleCharacter curChar : map.getCharacters()) {
            if (curChar != null) {
                int modifiedExp = exp;
                if (qid > -1) {
