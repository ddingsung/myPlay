...
public class MapleSnowball extends MapleEvent {
    ...
    public void startEvent() {
        for (int i = 0; i < 2; i++) {
            ...
            getMap(0).broadcastMessage(MaplePacketCreator.stopClock());
        }
    }
        ...
        public static final void hitSnowball(final MapleCharacter chr) {
            ...
            if (ball != null && !ball.isInvis()) {
                ...
                if (!snowman) {
                    ...
                    if (damage == 0) {
                        if (Math.random() < 0.2) {
                            chr.giveDebuff(MapleDisease.STUN, 1, 4000, MapleDisease.STUN.getDisease(), 1, (short) 0);
                        ...
                        if (ball.getPosition() == 255 || ball.getPosition() == 511 || ball.getPosition() == 767) { // Going to stage
                            ...
                            chr.getMap().broadcastMessage(MaplePacketCreator.serverNotice(6, "축하합니다! " + (team == 0 ? "story" : "maple") + " 팀이 눈덩이굴리기 이벤트에서 우승했습니다!"));
                        ...
                        if (ball.getSnowmanHP() <= 0) {
                            ...
                            for (MapleCharacter chrz : chr.getMap().getCharactersThreadsafe()) {
                                if ((ball.getTeam() == 0 && chr.getTruePosition().y < -80) || (ball.getTeam() == 1 && chr.getTruePosition().y > -80)) {
                                    chrz.giveDebuff(MapleDisease.SEDUCE, MobSkillFactory.getMobSkill(128, 1), (short) 0); //go left
                                }
                            }
                        }
                    }
