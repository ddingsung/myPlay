...
public class MobSkill {
    ...
    private int skillId, skillLevel, mpCon, spawnEffect, hp, x, y, count;
    ...
    public void applyEffect(MapleCharacter player, MapleMonster monster, boolean skill, short tDelay) {
        ...
        switch (skillId) {
            ...
            case 157:
                ...
                break;
            case 114:
                if (SystemUtils.getTimeMillisByDay(2013, 8, 30) > System.currentTimeMillis()) {
                    if (monster != null) {
                        if ((monster.getId() >= 8800000 && monster.getId() < 8810000)
                                || (monster.getId() >= 8810000 && monster.getId() <= 8810026)
                                || (monster.getId() >= 8810200 && monster.getId() <= 8810215)) {
                            break;
                        }
                    }
                }
            ...
            case 200:
                ...
                for (Integer mobId : getSummons()) {
                    ...
                    monster.getMap().spawnMonsterWithEffect(toSpawn, getSpawnEffect(), monster.getMap().calcPointBelow(new Point(xpos, ypos - 1)), tDelay);
                }
                break;
        }
        if (stats.size() > 0 && monster != null) {
            if (lt != null && rb != null && skill) {
                for (MapleMapObject mons : getObjectsInRange(monster, MapleMapObjectType.MONSTER)) {
                    ((MapleMonster) mons).applyMonsterBuff(stats, getSkillId(), getDuration(), this, reflection, tDelay);
                }
            } else {
                switch (monster.getStats().getId()) { //카니발 하드코딩 지우면 5대가 고자 주석쳐도 고자
                    case 9300127:
                    case 9300128:
                    case 9300129:
                    case 9300130:
                    ...
                    case 9300136:
                        switch (getSkillId()) {
                            case 140:
                                monster.applyMonsterBuff(stats, getSkillId(), 1200000, new MobSkill(getSkillId(), 10), reflection, tDelay);
                                break;
                            case 141:
                                monster.applyMonsterBuff(stats, getSkillId(), 1200000, new MobSkill(getSkillId(), 9), reflection, tDelay);
                                break;
                            default:
                                monster.applyMonsterBuff(stats, getSkillId(), getDuration(), this, reflection, tDelay);
                        }
                        break;
                    default:
                        monster.applyMonsterBuff(stats, getSkillId(), getDuration(), this, reflection, tDelay);
        ...
        if (count > 0) {
            //System.out.println("SkillId: " + skillId + " Level: " + skillLevel + " Count: " + count);
        }
        int counter = 0;
        if (disease != null && player != null) {
            if (lt != null && rb != null && skill && monster != null) {
                for (MapleCharacter chr : getPlayersInRange(monster, player)) {
                    if (disease == MapleDisease.SEDUCE && (chr.getMapId() == 240060200 || chr.getMapId() == 240060201)) {
                        if (chr.getMap().seduceOrder.size() < counter) {
                            break;
                        }
                        chr = chr.getMap().seduceOrder.get(counter);
                    }
                    if (chr == null) {
                        continue;
                    }
                    chr.giveDebuff(disease, this, tDelay);
                    counter++;
                    if (counter == count) {
                        break;
                    }
                ...
                player.giveDebuff(disease, this, tDelay);
            }
        }
        if (monster != null) {
            monster.setMp(Math.max(monster.getMp() - getMpCon(), Math.min(monster.getStats().getMp(), 500)));
        }
    ...
    private List<MapleCharacter> getPlayersInRange(MapleMonster monster, MapleCharacter player) {
        ...
        for (MapleCharacter chr : player.getMap().getCharactersThreadsafe()) {
            players.add(chr);
        }
