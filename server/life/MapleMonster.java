...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public Point reaPoint = new Point(-1, -1);
    private long damageLast = 0;
    ...
    public final int getMobExp() {
        if (stats.getId() == 9401012 || stats.getId() == 9401013) {
            return 0;
        }
        return stats.getExp();
    }
    ...
    public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime) {
        try {
            damage(from, damage, updateAttackTime, 0, System.currentTimeMillis());
        } catch (Exception e) {
            System.out.println(e);
        }
    }

    public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime, final int lastSkill, final long lastTime) {
        ...
        for (int i = 0; i < from.getMap().getAllMonster().size(); i++) {
            if ((from.getMap().getAllMonster().get(i).getPosition().y == from.getPosition().y) || (from.getMap().getAllMonster().get(i).getPosition().y - 1 == from.getPosition().y) || (from.getMap().getAllMonster().get(i).getPosition().y + 1 == from.getPosition().y)) {
                count++;
            }
        }
        ...
        if (stats.getSelfD() != -1) {
            ...
            MapleMonster Mob = this;//이지 본체
            if (Mob.hp > 0) {
                if (from.getMapId() == 105100400) {
                    MapleMonster Mob1 = map.getMonsterById(8830007);//이지 본체
                    MapleMonster Mob2 = map.getMonsterById(8830008);//이지 오른팔
                    MapleMonster Mob3 = map.getMonsterById(8830009);//이지 왼팔
                    if (Mob == Mob1) { //때리는 대상이 본체이면
                        if (Mob2 != null) {
                            Mob = Mob2;
                        } else if (Mob2 == null && Mob3 != null) {
                            Mob = Mob3;
                        } else if ((Mob2 == null && Mob3 == null)) {
                            Mob = Mob1;
                        }
                    }
                } else if (from.getMapId() == 105100300) {
                    MapleMonster Mob1 = map.getMonsterById(8830000);//노말 본체
                    MapleMonster Mob2 = map.getMonsterById(8830001);//노말 오른팔
                    MapleMonster Mob3 = map.getMonsterById(8830002);//노말 왼팔
                    if (Mob == Mob1) { //때리는 대상이 본체이면
                        if (Mob2 != null) {
                            Mob = Mob2;
                        } else if (Mob2 == null && Mob3 != null) {
                ...
                if (Mob.hp <= 0) {
                    ...
                    map.killMonster(Mob, from, true, false, (byte) 1, lastSkill);
                }
            }
        }
        startDropItemSchedule();
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final byte pty, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (ms != null) {
                exp += (int) (rawexp * (ms.getX() / 100.0));
            } else if (ms == null && (lastskillID == 4121003 || lastskillID == 4221003)) {
                MapleStatEffect skillEffect = SkillFactory.getSkill(lastskillID).getEffect(attacker.getTotalSkillLevel(lastskillID));
                exp += (int) (rawexp * (skillEffect.getX() / 100.0));
            }
            ms = stati.get(MonsterStatus.CHEMICAL_SHELL);
            if (ms != null) {
                exp += (int) (rawexp * (ms.getX() / 100.0));
            } else if (ms == null && lastskillID == 33121005) {
                MapleStatEffect skillEffect = SkillFactory.getSkill(lastskillID).getEffect(attacker.getTotalSkillLevel(lastskillID));
                if (!getStats().isBoss()) {
                    exp += (int) (rawexp * (skillEffect.getX() / 100.0));
                }
            ...
            if (holySymbol != null) {
                ...
                if (attacker.getBuffSource(MapleBuffStat.HOLY_SYMBOL) / 1000000 == 9 || attacker.getBuffSource(MapleBuffStat.HOLY_SYMBOL) == 2022033) { //GM
                    canFullHSEffect = true;
                }
                if (canFullHSEffect) { //파티원수 인원 체크여부
                    exp += rawexp * (1.0 + (holySymbol.doubleValue() / 100.0));
                } else {
                    exp += rawexp * (1.0 + (holySymbol.doubleValue() / 150.0));
                }

            }

            final Integer holySymbol2 = attacker.getBuffedValue(MapleBuffStat.HOLY_SYMBOL2);
            if (holySymbol2 != null) {
                boolean canFullHSEffect = false;
                if (attacker.getParty() != null) {
                    int ff = 0;
                ...
                if (attacker.getBuffSource(MapleBuffStat.HOLY_SYMBOL2) / 1000000 == 9 || attacker.getBuffSource(MapleBuffStat.HOLY_SYMBOL2) == 2022033) { //GM
                    canFullHSEffect = true;
                }
                if (canFullHSEffect) {//파티원수 인원 체크여부
                    exp += rawexp * (1.0 + (holySymbol.doubleValue() / 100.0));
            ...
            double expBuff = rawexp * (attacker.getStat().expBuff - 100) / 100.0;
            if (attacker.getStat().expBuff > 100 && expBuff < 1) {
                expBuff = 1;
            }
            exp += expBuff;
            if (GameConstants.isBeginnerJob(attacker.getJob()) && attacker.getLevel() <= 11) {
                attacker.gainExpMonster(0, rawexp, true, highestDamage, pty, stats.isPartyBonus(), stats.getPartyBonusRate());
            } else {
                attacker.gainExpMonster(rawexp, exp, true, highestDamage, pty, stats.isPartyBonus(), stats.getPartyBonusRate());
            }
        }
    ...
    public final int killBy(final MapleCharacter killer, final int lastSkill) {
        ...
        if (killer != null && killer.getPyramidSubway() != null) { //피라미드
            ...
            default: {
                for (final int i : toSpawn) {
                    ...
                    if (mob.getId() == 8800001 || mob.getId() == 8800002) {
                        ...
                        if (map.getId() == 280030000) { // 귀찮으니 하드코딩
                            if (mob.getPosition().y >= -214) {
                                mob.setPosition(new Point(-10, -215));
                            }
                        }
                    }
    ...
    public final void sendStatus(final MapleClient client) {
        ...
        if (poisons.size() > 0) {
            poisonsLock.readLock().lock();
            try {
                client.getSession().write(MobPacket.applyMonsterStatus2(this, poisons, 0, 0));
            } finally {
                poisonsLock.readLock().unlock();
            }
        }
...
//            }
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (eff != null && eff.getSourceId() == 5211005) {
        ...
        if (stats.isBoss() && status.getSkill() != 1201006) {
            if (stat == MonsterStatus.STUN || stat == MonsterStatus.SEAL || stat == MonsterStatus.FREEZE) {
                return;
            }
            if (checkboss && stat != (MonsterStatus.SPEED) && stat != (MonsterStatus.NINJA_AMBUSH) && stat != (MonsterStatus.WATK) && stat != (MonsterStatus.POISON) && stat != MonsterStatus.VENOM && stat != (MonsterStatus.BLIND) && stat != (MonsterStatus.MAGIC_CRASH)) {
                return;
            }
...
//        System.out.println("Cancel Time : " + aniTime);
        if (poison && getHp() > 1) {
            ...
            } else if (weapon_.getItemId() == 1472062 || weapon_.getItemId() == 1442078) {
                pDam = 300;
            } else if (weapon_.getItemId() == 1472086 || weapon_.getItemId() == 1382068 || weapon_.getItemId() == 1492037) {
                pDam = 120;
            }
...
//        }
        ...
        if (stat == MonsterStatus.POISON || (stat == MonsterStatus.VENOM && venomeff == null)) {
            ...
            try {
                ...
                if (con != null) {
                    map.broadcastMessage(con, MobPacket.applyMonsterStatus2(this, poisons, 0, (eff.getDOTTime() + from.getStat().dotTime)), getTruePosition());
                    con.getClient().getSession().write(MobPacket.applyMonsterStatus2(this, poisons, 0, (eff.getDOTTime() + from.getStat().dotTime)));
                } else {
                    map.broadcastMessage(MobPacket.applyMonsterStatus2(this, poisons, 0, (eff.getDOTTime() + from.getStat().dotTime)), getTruePosition());
                }
            } finally {
            ...
            if (con != null) {
                map.broadcastMessage(con, MobPacket.applyMonsterStatus1(this, venomeff, 0), getTruePosition());
                con.getClient().getSession().write(MobPacket.applyMonsterStatus1(this, venomeff, 0));
            } else {
                map.broadcastMessage(MobPacket.applyMonsterStatus1(this, venomeff, 0), getTruePosition());
            }
        } else {
            stati.put(stat, status);
            if (con != null) {
                map.broadcastMessage(con, MobPacket.applyMonsterStatus1(this, status, 0), getTruePosition());
                con.getClient().getSession().write(MobPacket.applyMonsterStatus1(this, status, 0));
            } else {
                map.broadcastMessage(MobPacket.applyMonsterStatus1(this, status, 0), getTruePosition());
            }
        }
    }
    ...
    public final void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int skillId, final long duration, final MobSkill skill, final List<Integer> reflection, int tDelay) {
        ...
        if (reflection.size() > 0) {
            ...
            this.reflectpack = MobPacket.applyMonsterStatus3(getObjectId(), effect, reflection, skill, tDelay);
            if (con != null) {
    ...
    public void usedSkill(final int skillId, final int level, long cooltime) {
        ...
        //System.out.println("skillId: " + skillId + " level: " + level + " cooltime: " + cooltime);
...
//            System.out.println("Returned 3");
    ...
    private final class SingleAttackerEntry implements AttackerEntry {
        ...
        public void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            if (chr != null && chr.isAlive()) {
                giveExpToCharacter(chr, baseExp, mostDamage, (byte) 1, lastSkill);
            }
        }
    ...
    private class PartyAttackerEntry implements AttackerEntry {
        ...
        public final void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            for (Entry<MapleCharacter, OnePartyAttacker> attacker : aaaattackers.entrySet()) {
                ...
                for (MaplePartyCharacter partychar : party.getMembers()) {
                    if (attacker.getKey().getLevel() - partychar.getLevel() <= 5
                            || stats.getLevel() - partychar.getLevel() <= 5) { //레벨 5차이
            ...
            for (Entry<MapleCharacter, Integer> expReceiver : expMap.entrySet()) {
                ...
                giveExpToCharacter(expReceiver.getKey(), expReceiver.getValue(), white, (byte) expMap.size(), lastSkill);
            }
        }
    ...
    public final void handleSteal(MapleCharacter chr) {
        ...
        if (level > 0 && !getStats().isBoss() && stolen == -1 && steal.getEffect(level).makeChanceResult()) {
            ...
            stolen = 1; //failed once, may not go again
        }
    }
    ...
    public final void cancelStatus(final MonsterStatus stat) {
        ...
        if (stat == MonsterStatus.HYPNOTIZE && mse.getSkill() == 30001062) {
    ...
    public LinkedList<MonsterStatusEffect> getPoisons() {
        return poisons;
    }

    public final void cancelSingleStatus(final MonsterStatusEffect stat) {
        if (stat == null || stat.getStati() == MonsterStatus.BURNED || !isAlive()) {
            return;
        }
