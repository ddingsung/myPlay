...
public class MapleQuestRequirement implements Serializable {
    ...
    public MapleQuestRequirement(MapleQuest quest, MapleQuestRequirementType type, ResultSet rse) throws SQLException {
        ...
        switch (type) {
            ...
            case info:
            case infoex: {
                cond = Integer.parseInt(rse.getString("stringStore"));
                intStore = Integer.parseInt(rse.getString("intStoresFirst"));
                stringStore = rse.getString("intStoresSecond");
                break;
            }
            case end: {
    ...
    public boolean check(MapleCharacter c, Integer npcid) {
        switch (type) {
            ...
            case info:
            case infoex:
                final MapleQuestStatus q = c.getQuestNoAdd(MapleQuest.getInstance(intStore));
                if (q == null) {
                    return false;
                }
                try {
                    if (cond == 2 && Integer.parseInt(q.getCustomData()) <= Integer.parseInt(stringStore)) {
                        return true;
                    }
                    if (!stringStore.equals(q.getCustomData())) {
                        return false;
                    }
                } catch (Exception e) {
                    return false;
            ...
            case end:
                ...
                for (ModifiedQuestTime modifiedQuestTime : modifiedQuestTimes) {
                    if (modifiedQuestTime.getQuestID() == quest.getId()) {
                        final Calendar cal = Calendar.getInstance();
                        final String timeStr = modifiedQuestTime.getEnd();
                        cal.set(Calendar.YEAR, Integer.parseInt(timeStr.substring(0, 4)));
                        cal.set(Calendar.MONTH, Integer.parseInt(timeStr.substring(5, 7)) - 1);
                        cal.set(Calendar.DATE, Integer.parseInt(timeStr.substring(8, 10)));
                        cal.set(Calendar.HOUR, Integer.parseInt(timeStr.substring(11, 13)));
                        cal.set(Calendar.MINUTE, Integer.parseInt(timeStr.substring(14, 16)));
        ...
        switch (type) {
            ...
            case mob:
                for (Pair<Integer, Integer> a : dataStore) {
                    ...
                    final int killReq = a.getRight();
                    if (c.getQuest(quest).getMobKills(mobId) < killReq) {
                        c.getQuest(quest).setMobKills(mobId, killReq - 1);
                        c.mobKilled(mobId, 0);
                    }
                }
                break;
