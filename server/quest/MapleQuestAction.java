    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            case exp:
                ...
                if (GameConstants.isBeginnerJob(c.getJob()) && c.getLevel() < 12) {
                    c.gainExp(intStore * c.getStat().questBonus, true, true, true);
                } else {
                    c.gainExp(intStore * c.getStat().questBonus * RateManager.QEXP, true, true, true);
                }
                break;
            case item:
                ...
                for (QuestItem item : items) {
                    ...
                    if (count <= 0) { // remove items
                        ...
                        if (GameConstants.getInventoryType(id) == MapleInventoryType.EQUIP) {
                            Equip eq = (Equip) MapleItemInformationProvider.getInstance().getEquipById(id, -1);
                            if (item.period > 0) {
                                eq.setExpiration((long) System.currentTimeMillis() + ((long) item.period * 60 * 1000)); //퀘스트는 분단위!
                            }
                            eq.setGMLog(c.getName() + "이 퀘스트를 통해 얻음: " + FileoutputUtil.CurrentReadable_Date());
                            MapleInventoryManipulator.addbyItem(c.getClient(), eq);
                        } else {
                            Item item2 = new client.inventory.Item(id, (byte) 0, count, (byte) 0);
                            if (item.period > 0) {
                                item2.setExpiration(System.currentTimeMillis() + ((long) item.period * 60 * 1000)); //퀘스트는 분단위!
                            }
                            item2.setGMLog(c.getName() + "이 퀘스트를 통해 얻음: " + FileoutputUtil.CurrentReadable_Date());
                            MapleInventoryManipulator.addbyItem(c.getClient(), item2);
                        }
                        c.getClient().getSession().write(MaplePacketCreator.getShowItemGain(id, count, true));
                        Item nitem = new client.inventory.Item(id, (byte) 0, count, (byte) 0);
                        nitem.setExpiration(System.currentTimeMillis() + ((long) item.period * 60 * 1000)); //퀘스트는 분단위!
                        ServerLogger.getInstance().logItem(LogType.Item.Quest, c.getId(), c.getName(), id, count, MapleItemInformationProvider.getInstance().getName(id), 0, "QuestStart - (" + quest.getId() + " / " + quest.getName() + ")");
                    }
                }
                break;
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        switch (type) {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count <= 0) { // remove items
                        ...
                        if (item.period > 0) {
                            ...
                            if (id == 1082254 || id == 1032075) {//머쉬룸 장갑 페페킹 장갑, 자유로운 영혼의 피어싱
                                MapleInventoryManipulator.addByIdPotential(c.getClient(), id, count, "", null, "획득한사람: " + c.getName() + " 기간: " + item.period + " on " + FileoutputUtil.CurrentReadable_Date(), true);
                                c.getClient().getSession().write(MaplePacketCreator.getShowItemGain(id, count, true));
                            } else {
                                MapleInventoryManipulator.addById(c.getClient(), id, count, "", null, "획득한사람: " + c.getName() + " 기간: " + item.period + " on " + FileoutputUtil.CurrentReadable_Date());
                                c.getClient().getSession().write(MaplePacketCreator.getShowItemGain(id, count, true));
                            }
                        }
                    }
            ...
            case nextQuest: {
                ...
                //c.getClient().getSession().write(MaplePacketCreator.updateQuestFinish(0, c.getQuest(quest).getNpc(), intStore));
                break;
            }
            case money: {
