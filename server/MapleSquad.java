...
public class MapleSquad {
    ...
    public static enum MapleSquadType {
        ...
        cwkPQ(6), bossbalrog(2), zak(1), chaoszak(3), horntail(1), easyht(1), pinkbean(3), nmm_squad(1), vergamot(1), dunas(1), nibergen_squad(1), dunas2(2), core_blaze(2), aufheben(2), cwkpq(10), tokyo_2095(1), vonleon(3), scartar(2), cygnus(3);
    ...
    public String getNextPlayer() {
        ...
        for (Pair<String, Long> chr : type.queue.get(ch)) {
            i++;
            sb.append(i).append("번 : ").append(chr.left);
            sb.append(" \r\n ");
        }
        sb.append("대기자 명단에 #e등록#n하시겠습니까? 이미 대기자 명단에 있는 경우에는 대기자 명단에서 #e취소#n됩니다.");
        return sb.toString();
    }
        ...
        if (join) {
            if (!containsMember(member) && !getAllNextPlayer().contains(member.getName())) {
                if (members.size() <= 30) {
                    ...
                    getLeader().dropMessage(5, member.getName() + "님이 원정대에 참가하였습니다");
                    return 1;
                }
                return 2;
            }
            ...
            if (containsMember(member)) {
                ...
                getLeader().dropMessage(5, member.getName() + "님이 원정대에서 탈퇴하였습니다.");
                return 1;
            }
            return -1;
        }
    ...
    public void acceptMember(int pos) {
        ...
        if (toadd != null && getChar(toadd) != null) {
            ...
            getChar(toadd).dropMessage(5, "원정대장이 귀하의 원정대 참여를 허가하였습니다.");
        }
    }
    ...
    public void banMember(int pos) {
        ...
        if (toban != null && getChar(toban) != null) {
            ...
            getChar(toban).dropMessage(5, "원정대장이 귀하를 원정대 제재 대상에 등록하였습니다.");
        }
    }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            case 0: {
                StringBuilder sb = new StringBuilder("원정대 인원 : ");
                sb.append("#b").append(members.size()).append(" #k \r\n").append("참가자 목록 : \r\n");
                int i = 0;
            ...
            case 2: {
                StringBuilder sb = new StringBuilder("원정대 목록 : ");
                sb.append("#b").append(members.size()).append(" #k\r\n ").append("참가자 목록 : \n\r");
                int i = 0, selection = 0;
            ...
            case 3: { //CWKPQ
                StringBuilder sb = new StringBuilder("직업 : ");
