...
public class AramiaFireWorks {
    ...
    private final void broadcastServer(final MapleCharacter c, final int itemid) {
        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "<" + c.getClient().getRealChannelName() + "채널> " + c.getMap().getMapName() + " : 필요한 만큼의 " + MapleItemInformationProvider.getInstance().getName(itemid) + "이 모두 모였습니다! "));
    }
