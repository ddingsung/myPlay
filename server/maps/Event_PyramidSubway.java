...
public class Event_PyramidSubway {
    ...
    public Event_PyramidSubway(final MapleCharacter c) {
        ...
        if (c.getParty() == null || c.getParty().getLeader().getId() == c.getId()) {
            ...
            energyBarDecrease = MapTimer.getInstance().register(new Runnable() {
                ...
                public void run() {
                    energybar -= (c.getParty() != null && c.getParty().getMembers().size() > 1 ? 2 : 1);
                    //c.dropMessage(6, bar + " / " + energybar);
                    if (broaded) {
                        broadcastUpdate(c);
                    ...
                    if (c.isGM()) {
                        c.dropMessage(5, "에너지가 " + energybar + " 남았다 나 죽어~!!");
                    }
                    if (energybar <= 0) { //why
                        fail(c);
                    }
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        final int time = (type == -1 ? 180 : (stage == 1 ? 120 : 180)) - 1;
        energybar = 100;
        bar = 0;
    ...
    public final void onKill(final MapleCharacter c) {
        ...
        if (type == -1) {
            switch (kill) {
                case 100:
                    MapleItemInformationProvider.getInstance().getItemEffect(2022616).applyTo(c);
                    break;
                case 300:
                    MapleItemInformationProvider.getInstance().getItemEffect(2022617).applyTo(c);
                    break;
            }
        } else {
            switch (kill) {
                case 250:
                    MapleItemInformationProvider.getInstance().getItemEffect(2022585).applyTo(c);
                    break;
                case 500:
                    MapleItemInformationProvider.getInstance().getItemEffect(2022586).applyTo(c);
                    break;
                case 1000:
                    MapleItemInformationProvider.getInstance().getItemEffect(2022587).applyTo(c);
                    break;
                case 2000:
                    MapleItemInformationProvider.getInstance().getItemEffect(2022588).applyTo(c);
                    break;
            }
        }
        if (type != -1) {
    ...
    public final void onChangeMap(final MapleCharacter c, final int newmapid) {
        if ((newmapid == 910330001 && type == -1) || (newmapid == 926010001 + type && type != -1)) {
            succeed(c);
        } else {
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        c.gainExp(exp * RateManager.PEXP, true, true, true);
        dispose(c);
    }
    ...
    public final void fail(final MapleCharacter c) {
        ...
        if (type == -1) {
            ...
            map = c.getClient().getChannelServer().getMapFactory().getMap(926020001 + type);
        }
    ...
    public final void dispose(final MapleCharacter c) {
        ...
        if (c.getBuffedValue(MapleBuffStat.PYRAMID_PQ) != null) {
            c.cancelEffectFromBuffStat(MapleBuffStat.PYRAMID_PQ);
        }
    }

    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().getSession().write(MaplePacketCreator.sendPyramidUpdate(bar));
    }
    ...
    public static boolean warpNextMap_Pyramid(final MapleCharacter c, final int difficulty) {
        ...
        final ChannelServer ch = c.getClient().getChannelServer();
        if (thisStage >= 4) {
            map = ch.getMapFactory().getMap(926010001 + difficulty);
            changeMap(c, map, 1, 200, 1);
            return true;
        }
