    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            case tristan_questMap: {
                c.getPlayer().getMap().startMapEffect("어둠 속에서 나타난 주니어발록을 물리쳐라!", 5120025);
                break;
            }
            case dojang_Eff: {
                ...
                if (c.getPlayer().getMapId() >= 921120005 && c.getPlayer().getMapId() <= 921120500) {
                    ...
                    if (c.getPlayer().getEventInstance() != null) {
                        ...
                        if (c.getPlayer().getEventInstance().getProperty("HP") == null) {
                            c.getPlayer().getEventInstance().setProperty("HP", "75000");
                        }
            ...
            case StageMsg_davy: {
                switch (c.getPlayer().getMapId()) {
                    ...
                    case 925100300:
                        ...
                        break;
                    case 925100400:
                        c.getPlayer().getMap().startMapEffect("열쇠로 문을 잠가주세요!", 5120020);
                        break;
                    case 925100500:
            ...
            case PRaid_D_Fenter: {
                ...
                if (eim != null && c.getPlayer().getCarnivalParty() != null) {
                    String teamStage = c.getPlayer().getCarnivalParty().getTeam() == 0 ? "Red_Stage" : "Blue_Stage";
                    String stagePro = eim.getProperty(teamStage);
                    int oriStage = stagePro != null ? Integer.parseInt(stagePro) : 0;
                    int newStage = c.getPlayer().getMapId() % 10 + 1;
                    if (newStage > oriStage) {
                        eim.broadcastPacket(MaplePacketCreator.sendGhostStatus(teamStage, String.valueOf(newStage)));
                        eim.setProperty(teamStage, String.valueOf(newStage));
                        eim.broadcastPlayerMsg(-1, (c.getPlayer().getCarnivalParty().getTeam() == 0 ? "레드(Red)" : "블루(Blue)") + "팀 스테이지 " + newStage + "진입!!");
                    }
                }
                break;
            }
...
//                c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(6160003), c.getPlayer().getPosition());
    ...
    public static void startScript_User(final MapleClient c, String scriptName) {
        ...
        if (c.getPlayer().isGM()) {
            c.getPlayer().dropMessage(6, "onUserEnter : " + onUserEnter.fromString(scriptName));
        }
        String data = "";
...
//                c.getSession().write(UIPacket.ShowWZEffect("Effect/Direction4.img/cannonshooter/out01"));
            ...
            case dangerInfo: { // 910400000
                c.getPlayer().getMap().resetFully();
                if (c.getPlayer().getQuestStatus(21733) == 1) {
                    c.getPlayer().getMap().spawnNpc(1204002, new Point(76, 120));
                    c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9300382), new Point(250, 120));
                    c.getPlayer().dropMessage(5, "정보가게는 인형사에게 점령당했다! 상황을 확인한 후 인형사를 물리치자!!!");
                }
                break;
            }
            case sealGarden: // 920030001
                c.getPlayer().getMap().resetFully();
            ...
            case PRaid_D_Enter: {
                EventInstanceManager eim = c.getPlayer().getEventInstance();
                if (eim != null) {
                    eim.setProperty(String.valueOf(c.getPlayer().getId()), String.valueOf(c.getPlayer().getMapId()));
                }
                break;
            }
            case PRaid_B_Enter: {
            ...
            case shammos_Base: {
                final MapleMonster shammos = MapleLifeFactory.getMonster(9300282);
                c.getPlayer().getMap().spawnMonsterWithEffectBelow(shammos, new Point(29, 154), 12);
                break;
            }
            case shammos_Result: {
                break;
            ...
            case Sky_StageEnter: {
                switch (c.getPlayer().getMapId()) {
                    case 240080100:
                        c.getPlayer().getMap().startMapEffect("플라잉 호크를 퇴치하라!", 5120026);
                        break;
                    case 240080200:
                        c.getPlayer().getMap().startMapEffect("플라잉 이글을 퇴치하라!", 5120026);
                        break;
                    case 240080300:
                        c.getPlayer().getMap().startMapEffect("플라잉 레드 와이번과 플라잉 블루 와이번을 퇴치하라!", 5120026);
                        break;
                    case 240080400:
                        c.getPlayer().getMap().startMapEffect("모든 몬스터를 퇴치하라!", 5120026);
                        break;
                    case 240080500:
                        c.getPlayer().getMap().startMapEffect("모든 몬스터를 퇴치하라!", 5120026);
                        break;
                    case 240080600:
                        c.getPlayer().getMap().startMapEffect("드래고니카를 무찌르고, 천공의 둥지로 진입하라!", 5120026);
                        break;
                    case 240080700:
                        c.getPlayer().getMap().startMapEffect("제한시간 내에 장애물을 돌파하고, 천공의 둥지로 진입하라!", 5120026);
                        break;
                    case 240080800:
                        c.getPlayer().getMap().startMapEffect("미나르 마을을 괴롭히는 드래곤 라이더를 무찔러라!", 5120026);
                        break;
                }
                break;
            }
            ...
            case aranTutorAlone: {
                if (c.getPlayer().getMapId() == 914000000) {
                    if (c.getPlayer().getQuestStatus(21000) == 0) {
                        c.getPlayer().updateInfoQuest(21002, "");
                    }
                }
                c.getSession().write(MaplePacketCreator.enableActions());
                break;
            ...
            case evanleaveD: {
                ...
                c.getSession().write(UIPacket.IntroLock(false));
                c.getSession().write(MaplePacketCreator.enableActions());
                if (GameConstants.isEvan(c.getPlayer().getJob()) && c.getPlayer().getMapId() == 100030100 && c.getPlayer().getQuestStatus(22012) != 2) {
                    c.getPlayer().forceCompleteQuest(22012);
                }
                break;
            }
            case dojang_Msg: {
            ...
            case mPark_stageEff:
                ...
                break;
            case TD_LC_title: {
                c.getSession().write(UIPacket.MapEff("temaD/enter/lionCastle"));
                break;
            }
            case TD_MC_title: {
                c.getSession().write(UIPacket.IntroDisableUI(false));
            ...
            case explorationPoint: {
                ...
                if (m != null && c.getPlayer().getLevel() >= m.level && c.getPlayer().getQuestStatus(m.questid) != 2) {
                    ...
                    if (changedd) {
                        ...
                        c.getPlayer().dropMessage(-1, "" + number + "/" + m.maps.length + "개 완료");
                        c.getPlayer().dropMessage(-1, "칭호 - " + String.valueOf(m) + " 탐험가 도전 중");
                    }
                }
                break;
            }
...
//                c.getSession().write(UIPacket.ShowWZEffect("Effect/Direction1.img/aranTutorial/ClickLirin"));
    ...
    private static void sendDojoClock(MapleClient c, int time) {
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getClock(time));
    }

    private static void sendDojoStart(MapleClient c, int stage) {
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.environmentChange("Dojang/start", 4));
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.environmentChange("dojang/start/stage", 3));
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.environmentChange("dojang/start/number/" + stage, 3));
        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.trembleEffect(0, 1));
    }
