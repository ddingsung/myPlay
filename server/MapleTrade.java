...
public class MapleTrade {
    ...
    public final void setMeso(final int meso) {
        ...
        if (chr.get().getMeso() >= meso) {
            chr.get().gainMeso(-meso, false, false, true);
...
//            chr.get().dropMessage(-2, chr.get().getName() + " : " + message);
    ...
    public static final void startTrade(final MapleCharacter c) {
        if (c.getTrade() == null) {
            ...
            c.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "이미 교환중입니다."));
        }
    }
        ...
        if (trade != null) {
            if (trade.getPartner() != null) {
                ...
                if (other != null && other.getTrade() != null) {
                    ...
                    other.setTrade(null);
                    other.dropMessage(5, c.getName() + " 님이 교환 요청을 거절하였습니다.");
                }
            }
