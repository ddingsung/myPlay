...
//	System.out.println("Delay [" + skillId + "] = " + (tickcount - lastAttackTickCount) + ", " + (Server_ClientAtkTickDiff - STime_TC));
            ...
            case SAME_DAMAGE:
                ...
                if (gm_message % 100 == 0) {
                    if (chrhardref.getJob() <= 500) {
                        ...
                        ServerLogger.getInstance().hackLog("[핵 추측] 캐릭터명 : " + MapleCharacterUtil.makeMapleReadable(chrhardref.getName()) + " | " + StringUtil.makeEnumHumanReadable(offense.name()) + (param == null ? "" : (" - " + param)));
                    }
                }
    ...
    public boolean canSmega() {
        if (lastSmegaTime + 60000 > System.currentTimeMillis() && chr.get() != null && !chr.get().isGM()) {
            return false;
        }
