...
public class MapleRing implements Serializable {
    ...
    public static MapleRing loadFromDb(int ringId, boolean equipped) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT * FROM rings WHERE ringId = ?"); // Get details..
            ps.setInt(1, ringId);
    ...
    public static void addToDB(int itemid, MapleCharacter chr, String player, int id, int[] ringId) throws Exception {
        Connection con = null;
        PreparedStatement ps = null;
        try {
    ...
    public static void changeItemIdByUniqueId(int itemid, int uid) throws SQLException {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("UPDATE rings SET itemid = ? WHERE ringId = ?");
            ps.setInt(1, itemid);
            ps.setInt(2, uid);
            ps.executeUpdate();
            ps.close();
    ...
    public static void DeleteRingForItemAndChrId(int chrid, int itemid) {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM rings WHERE partnerChrId = ? AND itemid = ?");
            ps.setInt(1, chrid);
            ps.setInt(2, itemid);
            rs = ps.executeQuery();
            if (!rs.next()) {
