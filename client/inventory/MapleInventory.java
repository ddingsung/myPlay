...
public class MapleInventory implements Iterable<Item>, Serializable {
    ...
    public int expirationById(int itemId) {
        for (Item item : inventory.values()) {
            if (item.getItemId() == itemId) {
                if (item.getExpiration() != -1) {
                    return (int) ((item.getExpiration() - System.currentTimeMillis()) / (24 * 60 * 60 * 1000L));
                }
            }
        }
        return 1;
    ...
    public boolean addFromDB(Item item) {
        ...
        return inventory.put(item.getPosition(), item) != null;
    }
    ...
    public boolean removeSlot(short slot) {
        return inventory.remove(slot) != null;
    }
