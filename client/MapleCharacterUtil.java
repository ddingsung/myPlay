...
public class MapleCharacterUtil {
    ...
    public static Pair<Short, Integer> getNXCodeInfo2(String code) {
        Pair<Short, Integer> ret = null;
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT `quantity`, `expiredate` FROM nxcode WHERE code = ?");
            ps.setString(1, code);
            rs = ps.executeQuery();
            if (rs.next()) {
                ret = new Pair<Short, Integer>(rs.getShort("quantity"), rs.getInt("expiredate"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
