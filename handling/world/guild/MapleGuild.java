...
public class MapleGuild implements java.io.Serializable {
    ...
    private boolean init = false, changed = false;
...
//            ps.close();
    ...
    public static final void loadAll() {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM bbs_replies");
            rs = ps.executeQuery();
...
//		changed_skills = false;
    ...
    public final void guildChat(final String name, final int cid, final String msg) {
        ServerLogger.getInstance().logChat(LogType.Chat.Guild, cid, name, msg, "[" + getName() + "]");
    ...
    public final void allianceChat(final String name, final int cid, final String msg) {
        ServerLogger.getInstance().logChat(LogType.Chat.Alliance, cid, name, msg, "[" + getName() + "]");
...
//        gainGP(500, true, mgc.getId());
        ...
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (mgcc.getId() == mgc.getId()) {
                    broadcast(MaplePacketCreator.memberLeft(mgcc, false));
                    bDirty = true;
                    members.remove(mgcc);
                    if (mgc.isOnline()) {
            ...
            while (itr.hasNext()) {
                ...
                if (mgc.getId() == cid && initiator.getGuildRank() < mgc.getGuildRank()) {
                    ...
                    if (mgc.isOnline()) {
                        ...
                        MapleCharacterUtil.sendNote(mgc.getName(), initiator.getName(), "길드에서 강퇴당하셨습니다.", 0);
    ...
    public final boolean changeARank(final int cid, final int newRank) {
        ...
        for (final MapleGuildCharacter mgc : members) {
            if (cid == mgc.getId()) {
                if (mgc.isOnline()) {
                    World.Guild.setGuildAndRank(cid, this.id, mgc.getGuildRank(), mgc.getGuildContribution(), newRank); //요기
                } else {
    ...
    public final void changeGuildLeader(final int cid) {
        if (changeRank(cid, 1) && changeRank(leader, 2)) {
            ...
            broadcast(MaplePacketCreator.guildLeaderChanged(id, leader, cid, allianceid));
            this.leader = cid;
            Connection con = null;
            PreparedStatement ps = null;
            try {
...
//                    gainGP((mgc.getLevel() - old_level) * mgc.getLevel(), false, mgc.getId());
    ...
    public final void gainGP(int amount, final boolean broadcast, final int cid) {
        ...
        gp += amount;
        level = calculateLevel();
        broadcast(MaplePacketCreator.updateGP(id, gp, level));
        if (broadcast) {
