...
public class ChannelServerThread extends Thread {
    ...
    private static Logger _log = Logger.getLogger(ChannelServerThread.class.getName());
    final int channel;
    final int port;
    ...
    public void run() {
        ...
        System.out.println("Channel Server " + channel + " Thread Started. at port " + port + ". Memory used : "
                + SystemUtils.getUsedMemoryMB() + "MB");

        while (!ChannelServer.getInstance(channel).isShutdown()) {
            try {
                Socket socket = _serverSocket.accept();
                System.out.println("Channel New Connection from "
