...
//    }
    private static void CloseMerchant(final MapleCharacter chr, final MapleClient c) {
        final IMaplePlayerShop merchant = chr.getPlayerShop();
        if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr) && merchant.isAvailable()) {
            //c.getSession().write(MaplePacketCreator.serverNotice(1, "프레드릭에서 아이템을 찾아가시기 바랍니다."));
            c.getSession().write(MaplePacketCreator.enableActions());
            merchant.removeAllVisitors(-1, -1);
            chr.setPlayerShop(null);
            merchant.closeShop(true, true, false);
            ((HiredMerchant) merchant).clearMessages();
            //c.getSession().write(PlayerShopPacket.shopTest(0x11, 0));
        } else if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr) && !merchant.isAvailable()) {
            //c.getSession().write(MaplePacketCreator.serverNotice(1, "프레드릭에서 아이템을 찾아가시기 바랍니다."));
            c.getSession().write(MaplePacketCreator.enableActions());
    ...
    private enum Interaction {
        ...
        REQUEST_TIE(0x30), //무승부
        ANSWER_TIE(0x31),
        GIVE_UP(0x32), //기권
        REQUEST_REDO(0x34), //무르기
        ANSWER_REDO(0x35), //무르기
        ...
        switch (action) { // Mode
                ...
                if (createType == 3) { // trade
                    ...
                    if (!canOpen || !chr.getMap().getPortalsInRange(chr.getTruePosition(), 15000).isEmpty()) {
                        ...
                        if (FieldLimitType.Minigames.check(chr.getMap().getFieldLimit()) || chr.getMap().allowPersonalShop()) {
                            chr.dropMessage(1, "이곳에서 미니게임을 개설할 수 없습니다.");
                            c.getSession().write(MaplePacketCreator.enableActions());
                            return;
                        }
                    }
                    ...
                    if (createType == 1 || createType == 2) {
                        ...
                        Item shop = c.getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte) slea.readShort());//1.2.6 변경
                        int itemid = slea.readInt();
                        if (shop == null || shop.getQuantity() <= 0 || shop.getItemId() != itemid || c.getPlayer().getMapId() < 910000001 || c.getPlayer().getMapId() > 910000022) {
                            return;
                        }
                        if (createType == 4) {
                    ...
                    if (ob instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (ips instanceof MaplePlayerShop && ((MaplePlayerShop) ips).isBanned(chr.getName())) {
                                chr.dropMessage(1, "해당 상점에 블랙리스트로 등록되어 이용할 수 없습니다.");
                                return;
                            } else {
            ...
            case CHAT: {
                ...
                if (chr.getTrade() != null) {
                    ...
                    ((HiredMerchant) ips).chat(c, message);
                    ...
                    ServerLogger.getInstance().logChat(chatType, c.getPlayer().getId(), c.getPlayer().getName(), message, etc);
            ...
            case MANAGE: {
                ...
                if (subpacket == 4 && type == 5) {
                    ...
                    if (ob instanceof HiredMerchant) {
                        ...
                        if (merchant.isOwner(chr) && merchant.isOpen() && merchant.isAvailable()) {
                            merchant.setOpen(false);
                            //merchant.broadcastToVisitors(MaplePacketCreator.serverNotice(1, "판매자가 물품을 정리하고 있습니다."));                                
                    ...
                    if (shop.getShopType() == 1 && HiredMerchantHandler.UseHiredMerchant(chr.getClient(), false)) {
                        ...
                        c.getSession().write(PlayerShopPacket.shopTest(0x11, 5));
                    }
            ...
            case SET_ITEMS: {
                ...
                if (quantity < 0) {
                    AutobanManager.getInstance().autoban(c, "Invalid Trade Using.");
                }
                if (targetSlot < 1 || targetSlot > 9) {
                    chr.dropMessage(1, "아이템을 황천으로 보내는 것은 불가능합니다.");
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
