...
public class DueyHandler {
    ...
    public static final void DueyOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (operation) {
            ...
            case 1: { // Start Duey, 13 digit AS
                ...
                int unk = slea.readInt(); // Theres an int here, value = 1  2A  01  A3 88 20 00     00 00 00 00
                //  9 = error
                slea.skip(4);
            ...
            case 3: { // Send Item
                ...
                if (amount < 0) {
                    AutobanManager.getInstance().autoban(c, "Invalid Duey Using.");
                    return;
                }
    ...
    public static boolean addNewItemToDb(int itemid, int quantity, int to, String from, String content, boolean ison) {
        if (itemid == 0) {
            return addItemToDB(null, itemid, from, to, ison, content, 1, true);
        } else {
            Item item = null;
            if (itemid / 1000000 == 1) {
