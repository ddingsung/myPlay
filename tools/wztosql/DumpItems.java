...
public class DumpItems {
    ...
    private MapleDataProvider item, string = MapleDataProviderFactory.getDataProvider(new File("wz/String.wz")), character;
    ...
    public void dumpItems() throws Exception {
        if (!hadError) {
            ...
            PreparedStatement ps_bonusExp = this.con.prepareStatement("INSERT INTO wz_itembonusexpdata(`itemid`, `incExpR`, `termStart`) VALUES (?, ?, ?)");
            try {
                dumpItems(psa, psr, ps, pse, ps_bonusExp);
            } catch (Exception e) {
                System.out.println(id + " quest.");
                e.printStackTrace();
                hadError = true;
            } finally {
                ps_bonusExp.executeBatch();
                ps_bonusExp.close();
                psr.executeBatch();
                psr.close();
                psa.executeBatch();
    ...
    public void dumpItems(MapleDataProvider d, PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, PreparedStatement ps_bonusExp, boolean charz) throws Exception {
            ...
            if (!topDir.getName().equalsIgnoreCase("Special") && !topDir.getName().equalsIgnoreCase("Hair") && !topDir.getName().equalsIgnoreCase("Face") && !topDir.getName().equalsIgnoreCase("Afterimage")) {
                for (MapleDataFileEntry ifile : topDir.getFiles()) {
                    ...
                    if (charz || topDir.getName().equalsIgnoreCase("Pet")) {
                        dumpItem(psa, psr, ps, pse, ps_bonusExp, iz);
                    } else {
                        for (MapleData itemData : iz) {
                            dumpItem(psa, psr, ps, pse, ps_bonusExp, itemData);
                        }
                    }
                }
            }
    ...
    public void dumpItem(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, PreparedStatement ps_bonusExp, MapleData iz) throws Exception {
        try {
        ...
        if (MapleDataTool.getIntConvert("spec/runOnPickup", iz, 0) > 0) {
            flags |= 0x2000;
        }
        ps.setInt(9, flags);
        ...
        if (id / 10000 == 180) {
            for (MapleData ninfo : iz) {
                try {
                    int idd = Integer.parseInt(ninfo.getName());
                    if (scrollReqs.length() > 0) {
                        scrollReqs.append(",");
                    }
                    scrollReqs.append(idd);
                } catch (Exception ex) {

                }
                System.out.println("PetEquip " + id + " -> " + scrollReqs);
            }
        }
        if (id / 10000 == 243) {
            int npc = MapleDataTool.getIntConvert("spec/npc", iz, 0);
            String script = MapleDataTool.getString("spec/script", iz, "");
            consumeItem.append(npc).append(':').append(script);
            System.out.println("ScriptItem " + id + " -> " + consumeItem);
        } else {
        ...
        dat = iz.getChildByPath("info/bonusExp");
        if (dat != null) {
            ps_bonusExp.setInt(1, id);
            for (MapleData bonusExp : dat) {
                ps_bonusExp.setInt(2, MapleDataTool.getIntConvert("incExpR", bonusExp, 0));
                ps_bonusExp.setInt(3, MapleDataTool.getIntConvert("termStart", bonusExp, 0));
                ps_bonusExp.addBatch();
            }
        }
        dat = iz.getChildByPath("info/replace");
        if (dat != null) {
    ...
    public void dumpItems(PreparedStatement psa, PreparedStatement psr, PreparedStatement ps, PreparedStatement pse, PreparedStatement ps_bonusExp) throws Exception {
        if (!update) {
            ...
            delete("DELETE FROM wz_itembonusexpdata");
            System.out.println("Deleted wz_itemdata successfully.");
        }
        System.out.println("Adding into wz_itemdata.....");
        dumpItems(item, psa, psr, ps, pse, ps_bonusExp, false);
        dumpItems(character, psa, psr, ps, pse, ps_bonusExp, true);
    ...
    protected final MapleData getStringData(final int itemId) {
        ...
        } else if ((itemId >= 1132000 && itemId < 1183000) || (itemId >= 1010000 && itemId < 1040000) || (itemId >= 1122000 && itemId < 1130000)) {
            data = eqpStringData;
            cat = "Eqp/Accessory";
