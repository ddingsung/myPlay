...
public class DumpMobSkills {
    ...
    public void dumpMobSkills() throws Exception {
        if (!hadError) {
            PreparedStatement ps = con.prepareStatement("INSERT INTO wz_mobskilldata(skillid, `level`, hp, mpcon, x, y, time, prop, `limit`, spawneffect,`interval`, summons, ltx, lty, rbx, rby, once, count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            try {
                dumpMobSkills(ps);
            } catch (Exception e) {
    ...
    public void dumpMobSkills(PreparedStatement ps) throws Exception {
        if (!update) {
            delete("TRUNCATE `wz_mobskilldata`");
        ...
        for (MapleData ids : skillz.getChildren()) {
            for (MapleData lvlz : ids.getChildByPath("level").getChildren()) {
                ...
                ps.setInt(18, MapleDataTool.getInt("count", lvlz, 0)); 
