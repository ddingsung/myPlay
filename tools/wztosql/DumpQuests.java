...
public class DumpQuests {
    ...
    public void dumpQuests() throws Exception {
        if (!hadError) {
            ...
            PreparedStatement ps = con.prepareStatement("INSERT INTO wz_questdata(questid, name, autoStart, autoPreComplete, viewMedalItem, selectedSkillID, blocked, autoAccept, autoComplete, timeLimit, timeLimit2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            PreparedStatement psa = con.prepareStatement("INSERT INTO wz_questactdata(questid, type, name, intStore, applicableJobs, uniqueid, stringStore) VALUES (?, ?, ?, ?, ?, ?, ?)");
            try {
    ...
    public void dumpQuests(PreparedStatement psai, PreparedStatement psas, PreparedStatement psaq, PreparedStatement ps, PreparedStatement psr, PreparedStatement psq, PreparedStatement psa) throws Exception {
        if (!update) {
            delete("TRUNCATE `wz_questdata`");
            delete("TRUNCATE `wz_questactdata`");
            delete("TRUNCATE `wz_questactitemdata`");
            delete("TRUNCATE `wz_questactskilldata`");
            delete("TRUNCATE `wz_questactquestdata`");
            delete("TRUNCATE `wz_questreqdata`");
            delete("TRUNCATE `wz_questpartydata`");
        ...
        final MapleData pinfoz = quest.getData("PQuest.img"); //1.2.6은 파퀘 없음
        ...
        for (MapleData qz : checkz.getChildren()) { //requirements first
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (reqData != null) {
                    ...
                    for (MapleData req : reqData.getChildren()) {
                        ...
                        } else if (req.getName().equals("info") || req.getName().equals("infoex")) {
                            int questNumber = MapleDataTool.getInt("infoNumber", reqData, id);
                            intStore1.append(questNumber);
                            if (req.getName().equals("info")) {
                                intStore2.append(MapleDataTool.getString("0", req, "E1R2R3O4R5C6O7D8E"));
                            } else {
                                intStore2.append(MapleDataTool.getString("0/value", req, "E1R2R3O4R5C6O7D8E"));
                                psr.setString(4, MapleDataTool.getString("0/cond", req, "0"));
                            }
                        }
                ...
                if (actData != null) {
                    ...
                    for (MapleData act : actData.getChildren()) {
                        ...
                        } else if (act.getName().equals("info")) {
                            psa.setInt(4, 0);
                            psa.setString(7, MapleDataTool.getString(act));
                        } else if (act.getName().equalsIgnoreCase("npcAct")) {
                            psa.setInt(4, 0);
                            psa.setString(7, MapleDataTool.getString(act));
                        }
                        psa.addBatch();
            ...
            if (infoData != null) {
                ...
                ps.setInt(10, MapleDataTool.getInt("timeLimit", infoData, 0));
                ps.setInt(11, MapleDataTool.getInt("timeLimit2", infoData, 0));
            } else {
                ps.setString(2, "");
                ps.setInt(3, 0);
                ps.setInt(4, 0);
            ...
            ps.addBatch();
            MapleData pinfoData = pinfoz.getChildByPath(String.valueOf(id)); //1.2.6은 파퀘 없음
